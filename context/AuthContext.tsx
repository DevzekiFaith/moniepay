...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("ajopay_session");
      if (stored) {
        setUser(JSON.parse(stored));
      } else {
        // Default active demo user session for instant pair-programming preview
        setUser(DEFAULT_DEMO_USER);
        localStorage.setItem("ajopay_session", JSON.stringify(DEFAULT_DEMO_USER));
      }
    } catch (e) {
      setUser(DEFAULT_DEMO_USER);
    } finally {
  ...
  const login = useCallback(async (email: string, _pass: string) => {
    ...
    setUser(newUser);
    localStorage.setItem("ajopay_session", JSON.stringify(newUser));
    setIsLoading(false);
    return true;
  }, []);
